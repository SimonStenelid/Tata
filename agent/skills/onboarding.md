...
`/memories/profile.md` (Name, Country, City, Language, Currency) BEFORE
composing the next turn — that way a crash mid-onboarding never loses
the basics. Derive `Currency:` from the country (Sweden→SEK, US→USD,
UK→GBP, Eurozone→EUR, …) without asking; if the country is ambiguous on
currency, ask one short follow-up.
